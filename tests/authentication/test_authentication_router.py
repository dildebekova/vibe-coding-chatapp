...
async def test_user_login_succeeds_given_valid_credentials(async_client: AsyncClient, bob_user: User):
    ...
    data = response.json()
    assert data["first_name"] == bob_user.first_name
    assert data["last_name"] == bob_user.last_name
    assert data["email"] == bob_user.email
    assert data["username"] == bob_user.username
    assert data["user_guid"] == str(bob_user.guid)
    assert data["user_image"] is None

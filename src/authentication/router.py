...
from src.authentication.cookie_utils import clear_auth_cookies, set_auth_cookies
...
async def login_with_google(
    ...
    base = UserLoginResponseSchema.model_validate(user).model_dump(by_alias=True)
    base["access_token"] = access_token
    base["refresh_token"] = refresh_token
    return LoginResponseSchema.model_validate(base)
...
async def get_new_access_token_from_refresh_token(
    response: Response,
    body: RefreshTokenBodySchema | None = None,
    ...
    set_auth_cookies(response, access_token=new_access_token, refresh_token=refresh_token)

    return {"access_token": new_access_token, "detail": "Access token has been successfully refreshed"}
